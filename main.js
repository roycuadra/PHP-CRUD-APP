...
addForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    const formData = new FormData(addForm); formData.append("add", 1);
...
updateForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    const formData = new FormData(updateForm); formData.append("update", 1);
...
const deleteUser = async (id) => {
    const data = await fetch(`action.php?delete=0&id= ${id}`, {
        method: "GET",
    });
